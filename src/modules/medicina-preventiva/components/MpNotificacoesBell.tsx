...
const ICONES: Record<MpNotificacaoTipo, typeof Bell> = {
    consulta: Calendar,
    exame: FlaskConical,
    sintoma: Activity,
    receita: Pill,
...
const CORES: Record<MpNotificacaoTipo, string> = {
    consulta: "bg-emerald-50 text-emerald-600",
    exame: "bg-blue-50 text-blue-600",
    sintoma: "bg-rose-50 text-rose-600",
    receita: "bg-violet-50 text-violet-600",
    rotina: "bg-amber-50 text-amber-600",
    sistema: "bg-[#e3f1eb] text-[#5ba58c]",
};

export function MpNotificacoesBell() {
    const navigate = useNavigate();
    const [open, setOpen] = useState(false);
    const [lista, setLista] = useState<MpNotificacao[]>([]);
    ...
    useEffect(() => {
        ...
        getMpScope().then((scope) => {
            if (!ativo || !scope) return;
            const filtro = scope.titularId
                ? `titular_id=eq.${scope.titularId}`
                : `auth_id=eq.${scope.authId}`;
            channel = supabase
                .channel("mp_notificacoes_bell")
                .on(
                    "postgres_changes",
                    { event: "*", schema: "public", table: "mp_notificacoes", filter: filtro },
                    recarregar
                )
                .subscribe();
        });
    ...
    async function handleClick(n: MpNotificacao) {
